...
//                     'follow_user'  => AppServAuth::$userInfo['user']['username'],
            ...
            if (!empty($photoes)) {
                $attachModel=D('Attach');
                $photo = $attachModel->getPathOne($saleId);
                //若有上传车牌图片，将车牌图片信息插入mbs_check_car_photo
                foreach ($photoes as $key => $value) {
                    if ($photo[$value['type']] != 'http://img.273.com.cn/' && in_array($value['type'], array(97, 98, 99))) {
                        $CheckCarPhotoInfo['car_id'] = $saleId;
                        $CheckCarPhotoInfo['photo'] = $photo[$value['type']];
                        $CheckCarPhotoInfo['photo_type'] = 100 - $value['type'];
                        $CheckCarPhotoInfo['follow_user'] = $carData['follow_user'];
                        $ret[] = MbsCheckCarPhotoInterface::add($CheckCarPhotoInfo);
                    }
                } 
            }
            if (isset($params['car_number']) && $params['car_number']) {
    ...
    private function _format($data, $type = '', $query_type = '') {
        ...
        foreach ($data as $k=>$v) {
            ...
            if ($type != 'buy') {
                ...
                $carPhotos = MbsCheckCarPhotoInterface::getCheckInfoByCarId(array('car_id' => $v['id']));
                $status = array();
                if (!empty($imageList)) {
                    foreach ($imageList as $key => $images) {
                        if (!isset($images['is_plate'])) {
                            ...
                            foreach($carPhotos as $m => $n) {
                                if($n['photo_type'] == 1 && $images['type'] == 99) {
                                    $h = $m;
                                }
                                if($n['photo_type'] == 2 && $images['type'] == 98) {
                                    $h = $m;
                                }
                                if($n['photo_type'] == 3 && $images['type'] == 97) {
                                    $h = $m;
                                }
                            }
    ...
    public function checkMessage($params) {
        ...
        $data['reason'] = '1, 3, 4, 5, 7, 8, 98, 129';
